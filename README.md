...
## 更新日志
- 未发布
  - **优化**：文件访问上传维护区的新建文件夹和退出登录按钮背景色加深，提高按钮辨识度。
  - **优化**：文件访问上传维护区分割线改为整行对齐，避免上传和新建文件夹操作之间出现断线。
  - **优化**：Web 页面版权年份自动显示当前年份，避免每年手动维护。
  - **优化**：文件访问上传维护区将上传作为主操作，新建文件夹改为弹层输入，并用页面内提示替代上传/删除后的生硬浏览器提示。
  - **优化**：文件访问目录页路径改为可点击面包屑导航，并进一步收紧上传维护区的管理操作布局。
  - **优化**：文件访问上传控件的选择文件按钮样式，使其与页面按钮风格保持一致。
  - **优化**：登录页用户名或密码错误、验证码错误提示改为 i18n 文案；文件访问上传登录文案统一为管理密码。
  - **优化**：文件访问目录页登录按钮固定在标题栏右上角，不再单独占用一行。
  - **优化**：文件访问目录页右上角登录入口改为与标题区域网格对齐，登录失败时在弹层内显示友好提示。
  - **优化**：文件访问目录页未登录时仅在右上角显示紧凑登录入口，避免上传维护面板占用下载浏览空间。
  - **优化**：文件访问目录页放大“文件浏览”标题，并移除“当前路径”标题文字，仅保留突出显示的实际路径。
  - **优化**：文件访问目录页将主标题改为当前路径，并突出显示当前目录路径。
  - **优化**：文件访问目录页默认使用中文文案，路径移到左上角，上传维护区输入框与按钮底部对齐。
  - **优化**：美化文件访问目录页和上传维护面板，提升路径展示、表格层次、管理按钮和移动端布局体验。
  - **新增**：文件访问模式支持可选上传管理，配置 `allow_upload=true` 和 `upload_password` 后，输入管理密码可创建文件夹、上传文件和删除文件/文件夹。
