...
- 2026-09-11
  ...
  - **修复**：目标熔断拨号改用带 generation 的许可令牌回报结果，较早发起的旧拨号即使稍后成功，也不会误关闭新一代已经打开的熔断状态。
  - **优化**：目标熔断状态增加机会式回收，空闲状态默认保留 15 分钟并以 4096 条为软上限；在途拨号、半开探测和仍处于隔离窗口的状态不会被回收。
  - **新增**：管理端增加 `/index/targetmetrics` 只读 JSON 接口，按 target 展示 nps LocalProxy 的熔断状态、在途拨号、成功/失败/超时/快速失败/打开次数及最近错误；远程 npc 指标保留在各 npc 进程内，不包含在此接口中。
...
- 2026-09-04  v0.27.28
  ...
  - **优化**：目标连接熔断期间的快速失败不再在普通告警日志里反复输出动态倒计时，只在目标刚被隔离等关键事件记录稳定日志，降低高并发请求命中异常 target 时的日志噪声。
  - **优化**：目标连接熔断失败计数增加 10 秒窗口，只有同一 target 在 10 秒内连续失败 3 次才触发隔离，超过 10 秒的旧失败不再累计，降低偶发网络抖动导致误熔断的概率。
  - **优化**：目标连接熔断从固定等待调整为自适应半开探测，连续失败后 5 秒起步、失败逐步退避并最多隔离 30 秒，窗口到期只放行一个真实探测，成功后立即恢复，兼顾高并发保护和后端恢复后的快速回切。
...
- 2026-08-14  v0.27.27
  ...
  - **修复**：从某个客户端的 TCP 代理列表点击新增时，复用编辑/克隆代理的同一套表单，保持客户端 ID、服务端 IP、端口选择器和字段布局一致。

- 2026-08-03  v0.27.26
  - **修复**：新增或编辑代理时保留页面已回填的客户端 ID，避免初始化本地代理选项时将其清空并在保存时误报客户端不存在。

- 2026-08-01  v0.27.25
  - **修复**：HTTP 代理按标准清理仅适用于相邻连接的 hop-by-hop 请求头，不再将入口代理的 `Connection: close` 传递给上游；同时保留下游关闭语义和 `TE: trailers`，避免 gzip、chunked 响应已返回正文但连接无法正确结束，导致页面和静态资源持续加载。

- 2026-07-27  v0.27.24
  - **修复**：HTTP 代理返回 502 或 504 时生成与状态码一致的错误页面，502 正文显示更明确的 `Bad Upstream` 并附带本轮目标地址，避免响应状态已经是上游错误但页面正文仍显示 `[404] Url not match`，便于快速定位异常后端。

- 2026-07-25  v0.27.23
  - **优化**：后台、登录和注册页面的版权区域同步展示当前 NPS 版本号，版本信息直接跟随服务端版本常量更新。
  - **新增**：全局参数页面提供管理员专属的红色 NPS 重启按钮和危险操作二次确认；系统服务模式交由服务管理器重启，直接运行模式安全替换或重新拉起进程，并在页面中检测新实例恢复。
