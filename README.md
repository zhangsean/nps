...
## 更新日志
- 未发布
  - **新增**：Web 管理端“全局参数”页面集成管理员专属的 `nps.conf` 在线编辑器，保存前校验并生成备份；连接超时、目标重试、上游响应超时、权限开关和全局 IP 黑名单可立即热加载。
  - **调整**：全局 IP 黑名单改由 `global_black_ip_list` 配置项保存，旧 `global.json` 数据会自动迁移；配置页面移除独立入口并精简生效说明。
  - **优化**：配置编辑器使用可视区域内的独立滚动，桌面端消除页面外层重复滚动条，左上角直接显示配置文件完整路径；保存按钮和精简后的“热生效配置 / 重启生效配置”说明固定在右侧，编辑长配置时无需滚动页面寻找操作区。
  - **优化**：管理端采用固定应用壳布局，左侧导航保持在视口内，仅右侧核心内容区域滚动。
  - **安全**：端口、协议、证书路径等启动配置仅保存并明确归入“重启生效配置”，避免误报为热更新成功。

- 2026-07-24  v0.27.22
  - **优化**：文件管理上传改为 8 MB 分片传输，展示总进度、单文件进度、速度和预计剩余时间；支持暂停、失败自动重试，以及重新选择同一文件后的断点续传，降低大文件长请求触发 504 的概率。
  - **新增**：文件访问隧道增加 `browse_url` 浏览地址配置，浏览页路径展示对外地址，并为每个文件提供“链接”按钮；复制链接保留中文路径原文。
