...
## 更新日志
- 未发布
  - **新增**：文件访问模式支持可选上传管理，配置 `allow_upload=true` 和 `upload_password` 后，输入上传密码可创建文件夹、上传文件和删除文件/文件夹。
  ...
  - **新增**：文件访问模式支持类似 nginx autoindex 的目录浏览，Web 新增/编辑隧道可选择 `file`，文件目录默认 `/files`；选择代理到服务端本地时客户端 ID 自动固定为 `1`，并由 nps 直接提供本地目录访问。
