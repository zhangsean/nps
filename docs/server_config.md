...
log_level|日志输出级别
http_access_log_path|HTTP/HTTPS 代理访问日志文件路径，输出 JSON Lines，包含时间戳、请求方法、URL、响应状态码、请求/响应字节数和处理耗时；LocalProxy 目标连接重试时会输出 `event=target_connect_retry` 事件；为空表示关闭
