...
#### 文件访问模式
利用 nps 提供一个公网可访问的文件目录浏览服务，默认文件目录为 `/files`。

在 Web 管理中新增文件访问隧道时，如果选择“代理到服务端本地”，客户端 ID 会自动使用 `1`，访问公网 `ip:server_port/` 即可浏览服务端 `/files` 目录。

访问客户端所在机器的文件时，推荐使用客户端配置文件模式：
...
upload_password|管理密码，`allow_upload=true` 时必须配置
